...
public inductive BasisCircuitOverRegister : (TypeBasisRegister → Type 1) where
  ...
  | RegisterSwap {R1 R2 : TypeBasisRegister} (iso : R1 ≅ R2) (c : BasisCircuitOverRegister R1) : BasisCircuitOverRegister R2
...
public noncomputable def BasisCircuitAreSimpleCircuit {R : TypeBasisRegister} (c : BasisCircuitOverRegister R)
  ...
  | RegisterSwap iso c => SimpleCircuitOverRegister.RegisterSwap (BasisRegIsoToQuantRegIso iso) (BasisCircuitAreSimpleCircuit c)
...
public theorem BasisGateReprIsSimpleGateRepr {R : TypeBasisRegister} (c : BasisCircuitOverRegister R) :
  ...
  by induction c with
    ...
    | RegisterSwap iso c c_ih => unfold SyntacticCircuit.SimpleCircuitGateRepr; simp; rw[c_ih];
    | Gate g => rfl
