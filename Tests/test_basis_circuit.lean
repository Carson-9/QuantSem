...
noncomputable def a_bad_recursive_circuit (n : ℕ) : SimpleCircuit := match n with
...
noncomputable def a_correct_recursive_circuit (n : ℕ) : SimpleCircuit := match n with
  |0 => ⟨_, a_simple_circuit_part⟩
  |Nat.succ k =>
    ⟨_, HorizontalComp
...
noncomputable def reparenthesing_iso (X Y Z : BasisRegister) :
  (X ⊗ᵣ (Y ⊗ᵣ Z)) ≅ ((X ⊗ᵣ Y) ⊗ᵣ Z) := _

noncomputable def fixed_the_bad_recursive_circuit (n : ℕ) : SimpleCircuit := match n with
  |0 => ⟨_, a_simple_circuit_part⟩
  |Nat.succ k =>
    ⟨_,
    HorizontalComp (
...
noncomputable def turning_a_two_qubit_circuit_in_a_quadradit_circuit : BasisCircuitOverRegister (ComplexSpace 4) :=
  ...
  iso : ((BasisRegisterToQuantumRegister QubitSpace) ⊗ᵣ (BasisRegisterToQuantumRegister QubitSpace) : SyntacticRegister.QuantumRegister) ≅ (ComplexSpace 4) := _ -- todo, need to fix axiom of choice thingy


#check (SimpleCircuitGetShape (a_correct_recursive_circuit 4))
